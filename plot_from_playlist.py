...
fig_height = max(12, num_tracks * 0.35 + 2)
fig = plt.figure(figsize=(22, fig_height))
# Remove default suptitle spacing by setting y very close to 1.0
fig.text(0.02, 0.99, playlist_name, fontsize=16, fontweight='bold', ha='left', va='top')
...
gs = GridSpec(2, 14, figure=fig, height_ratios=[0.06, 1], hspace=0.01, wspace=0.4)

# Row 1: Headers spanning multiple columns
# Energy Profile header (spans columns 1-5: RMS, Percussive, Centroid, Flux, RMS Std)
ax_energy_header = fig.add_subplot(gs[0, 1:6])
...
# Rhythmic Intensity header (spans columns 6-7: Onset, Attack)
ax_rhythm_header = fig.add_subplot(gs[0, 6:8])
...
# Spectral Balance header (spans columns 8-9: HF, Bass)
ax_spectral_header = fig.add_subplot(gs[0, 8:10])
...
ax4.barh(track_indices, df['centroid_mean'], color=COLOR_CENTROID, alpha=0.8, height=0.8)
ax4.set_xlabel('Hz', fontweight='bold', fontsize=9)
ax4.set_title('Brightness', fontsize=10, pad=8)
ax4.tick_params(labelleft=False)
ax4.grid(axis='x', alpha=0.3, linestyle='--')

# 5. Spectral Flux
ax5 = fig.add_subplot(gs[1, 4], sharey=ax1)
ax5.barh(track_indices, df['flux_mean'], color=COLOR_FLUX, alpha=0.8, height=0.8)
ax5.set_xlabel('Flux', fontweight='bold', fontsize=9)
ax5.set_title('Spectral Flux', fontsize=10, pad=8)
ax5.tick_params(labelleft=False)
ax5.grid(axis='x', alpha=0.3, linestyle='--')

# 6. Energy Variation (RMS Std Dev)
ax6 = fig.add_subplot(gs[1, 5], sharey=ax1)
ax6.barh(track_indices, df['rms_std'], color='#7f7f7f', alpha=0.8, height=0.8)
ax6.set_xlabel('Std Dev', fontweight='bold', fontsize=9)
ax6.set_title('Energy Variation', fontsize=10, pad=8)
ax6.tick_params(labelleft=False)
ax6.grid(axis='x', alpha=0.3, linestyle='--')

# RHYTHMIC INTENSITY GROUP
# 7. Onset Rate
ax7 = fig.add_subplot(gs[1, 6], sharey=ax1)
ax7.barh(track_indices, df['onset_rate'], color=COLOR_ONSET, alpha=0.85, height=0.8)
ax7.set_xlabel('Events/sec', fontweight='bold', fontsize=9)
ax7.set_title('Onset Rate', fontsize=10, pad=8)
ax7.tick_params(labelleft=False)
ax7.grid(axis='x', alpha=0.3, linestyle='--')

# 8. Attack Sharpness
ax8 = fig.add_subplot(gs[1, 7], sharey=ax1)
ax8.barh(track_indices, df['attack_sharpness'], color=COLOR_ATTACK, alpha=0.85, height=0.8)
ax8.set_xlabel('Sharpness', fontweight='bold', fontsize=9)
ax8.set_title('Attack Sharpness', fontsize=10, pad=8)
ax8.tick_params(labelleft=False)
ax8.grid(axis='x', alpha=0.3, linestyle='--')

# SPECTRAL BALANCE GROUP
# 9. High Frequency Ratio
ax9 = fig.add_subplot(gs[1, 8], sharey=ax1)
ax9.barh(track_indices, df['hf_ratio'], color=COLOR_HF, alpha=0.85, height=0.8)
ax9.set_xlabel('Ratio', fontweight='bold', fontsize=9)
ax9.set_title('HF Ratio (≥6kHz)', fontsize=10, pad=8)
ax9.tick_params(labelleft=False)
ax9.grid(axis='x', alpha=0.3, linestyle='--')

# 10. Bass Ratio
ax10 = fig.add_subplot(gs[1, 9], sharey=ax1)
ax10.barh(track_indices, df['bass_ratio'], color=COLOR_BASS, alpha=0.85, height=0.8)
ax10.set_xlabel('Ratio', fontweight='bold', fontsize=9)
ax10.set_title('Bass Ratio (<200Hz)', fontsize=10, pad=8)
